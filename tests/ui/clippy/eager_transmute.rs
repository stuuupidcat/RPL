...
fn f(op: u8, op2: Data, unrelated: u8) {
    ...
    //~^ eager_transmute
    let _: Option<Opcode> =
        (op > 0 && op < 10 && unrelated == 0).then_some(unsafe { std::mem::transmute(op) });
    //~^ eager_transmute

    // lint even when the transmutable goes through field/array accesses
    let _: Option<Opcode> =
        (op2.foo[0] > 0 && op2.foo[0] < 10).then_some(unsafe { std::mem::transmute(op2.foo[0]) });
    //~^ eager_transmute
    ...
    //~^ eager_transmute
    let _: Option<Opcode> =
        ((1..=3).contains(&op) || op == 4).then_some(unsafe { std::mem::transmute(op) });
    //~^ eager_transmute
    let _: Option<Opcode> = (1..3)
        .contains(&op)
        .then_some(unsafe { std::mem::transmute(op) });
    //~^ eager_transmute
    let _: Option<Opcode> = (1..)
        .contains(&op)
        .then_some(unsafe { std::mem::transmute(op) });
    //~^ eager_transmute
    let _: Option<Opcode> = (..3)
        .contains(&op)
        .then_some(unsafe { std::mem::transmute(op) });
    //~^ eager_transmute
    let _: Option<Opcode> = (..=3)
...
fn niche_tests(v1: u8, v2: NonZero<u8>, v3: NonZeroNonMaxU8) {
    ...
    let _: Option<NonMaxU8> =
        (v2 < NonZero::new(255u8).unwrap()).then_some(unsafe { std::mem::transmute(v2) });
    //~^ eager_transmute
    //~| eager_transmute
