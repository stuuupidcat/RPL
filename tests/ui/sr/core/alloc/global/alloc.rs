...
fn main() {
    unsafe {
        ...
        let _ = GlobalAlloc::alloc(&TestAllocator, unchecked);
        //~^ unsafe_numeric_precondition

        let checked = Layout::from_size_align(0, 8).unwrap();
        let _ = GlobalAlloc::alloc(&TestAllocator, checked);
        //~^ unsafe_numeric_precondition

        let new = Layout::new::<[u8; 0]>();
        let _ = GlobalAlloc::alloc(&TestAllocator, new);
        //~^ unsafe_numeric_precondition

        let array = Layout::array::<u8>(0).unwrap();
        let _ = GlobalAlloc::alloc(&TestAllocator, array);
        //~^ unsafe_numeric_precondition

        let nonzero = Layout::from_size_align_unchecked(8, 8);
        let _ = GlobalAlloc::alloc(&TestAllocator, nonzero);
    }
}
