pub mod u8_to_t {
        ...
        fn visit_fn(
            ...
            if self.tcx.visibility(def_id).is_public() && self.tcx.is_mir_available(def_id) {
                ...
                if let mut patterns_reversed_paras = MirPatternBuilder::new(self.pcx)
                    && let pattern_reversed_para = pattern_misordered_param(&mut patterns_reversed_paras)
    ...
    fn pattern_misordered_param(patterns: &mut pat::MirPatternBuilder<'_>) -> PatternMisorderedParam {
        mir! {
            meta!{$T:ty}
