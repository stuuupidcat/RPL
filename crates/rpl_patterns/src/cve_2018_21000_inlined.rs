    ...
    fn visit_fn(
        ...
        if self.tcx.visibility(def_id).is_public() && self.tcx.is_mir_available(def_id) {
            ...
            if let mut patterns_reversed_paras = PatternsBuilder::new(&self.tcx.arena.dropless)
                ...
                && let Some(vec_constructing_stmt) = matches[pattern_reversed_para.vec_constructing_stmt]
                && let vec_constructing_stmt = vec_constructing_stmt.span_no_inline(body)
            {
                debug!(?vec_constructing_stmt);
                self.tcx
                    .dcx()
                    .emit_err(crate::errors::ReversedParameters { vec_constructing_stmt });
            }
        }
    }
}

struct PatternReversedPara {
    vec_constructing_stmt: pat::Location,
}

#[rpl_macros::mir_pattern]
fn pattern_reversed_para(patterns: &mut pat::PatternsBuilder<'_>) -> PatternReversedPara {
    mir! {
        meta!{$T:ty}
    ...
    PatternReversedPara {
        vec_constructing_stmt: to_vec_stmt,
    }
}
