...
use super::{MetaVars, MirPattern, Path, Pattern, RawStatement, Ty};
...
impl<'pcx> Fn<'pcx> {
    pub fn from(pair: &'pcx pairs::Fn<'_>, pcx: PatCtxt<'pcx>) -> Self {
        ...
        let raw_stmts = stmts.into_iter().map(|stmt| RawStatement::from(stmt, pcx));
...
impl<'pcx> Param<'pcx> {
    ...
    pub fn from(param: &'pcx pairs::FnParam<'_>, pcx: PatCtxt<'pcx>) -> (Option<Self>, bool) {
        match param.deref() {
...
impl<'pcx> Params<'pcx> {
    pub fn from(pair: &'pcx pairs::FnParamsSeparatedByComma<'_>, pcx: PatCtxt<'pcx>) -> Self {
