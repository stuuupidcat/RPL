...
impl<'pcx> Ty<'pcx> {
    ...
    pub fn from(ty: &'pcx pairs::Type<'_>, pcx: PatCtxt<'pcx>) -> Self {
        match ty.deref() {
    ...
    pub fn from_fn_ret(ty: &'pcx pairs::FnRet<'_>, pcx: PatCtxt<'pcx>) -> Self {
...
impl RegionKind {
    pub fn from(region: &pairs::Region<'_>) -> RegionKind {
        match region.get_matched().1.deref() {
...
impl<'pcx> GenericArgKind<'pcx> {
    fn from(arg: &'pcx pairs::GenericArgument<'_>, pcx: PatCtxt<'pcx>) -> GenericArgKind<'pcx> {
        match arg.deref() {
...
impl<'pcx> Path<'pcx> {
    pub fn from(path: &'pcx pairs::Path<'_>, pcx: PatCtxt<'pcx>) -> Self {
        ...
        items.extend(path.segments.iter().map(|seg| {
            let ident = seg.get_matched().0;
            match ident.deref() {
...
impl<'pcx> GenericArgsRef<'pcx> {
    pub fn from_path(args: &'pcx pairs::Path<'_>, pcx: PatCtxt<'pcx>) -> Self {
    ...
    pub fn from_angle_bracketed_generic_arguments(
        args: &'pcx pairs::AngleBracketedGenericArguments<'_>,
        pcx: PatCtxt<'pcx>,
    ) -> Self {
...
impl<'pcx> PathWithArgs<'pcx> {
    pub fn from_path(path: &'pcx pairs::Path<'_>, pcx: PatCtxt<'pcx>) -> Self {
        let args = GenericArgsRef::from_path(path, pcx);
        let path = Path::from(path, pcx);
        Self { path, args }
    }

    pub fn from_type_path(path: &'pcx pairs::TypePath<'_>, pcx: PatCtxt<'pcx>) -> Self {
    ...
    pub fn from_lang_item(lang_item: &'pcx pairs::LangItemWithArgs<'_>, pcx: PatCtxt<'pcx>) -> Self {
    ...
    pub fn from_path_or_lang_item(path_or_lang_item: &'pcx pairs::PathOrLangItem<'_>, pcx: PatCtxt<'pcx>) -> Self {
...
impl<'pcx> Const<'pcx> {
    pub fn from(konst: &'pcx pairs::Konst<'_>) -> Self {
        match konst.deref() {
    ...
    pub fn from_integer(int: &'pcx pairs::Integer<'_>) -> Self {
        Self::Value(IntValue::from_integer(int))
    }

    pub fn from_gconst(konst: &'pcx pairs::GenericConst<'_>) -> Self {
