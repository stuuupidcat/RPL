...
pub(crate) fn nullop_from_pair<'pcx>(pair: &'pcx pairs::MirNullOp<'_>) -> mir::NullOp<'pcx> {
    match pair.deref() {
...
pub(crate) fn collect_operands<'pcx>(
    operands: &'pcx Option<pairs::MirOperandsSeparatedByComma<'_>>,
    pcx: PatCtxt<'pcx>,
) -> Vec<Operand<'pcx>> {
    if let Some(operands) = operands {
        collect_elems_separated_by_comma!(operands)
            .map(|operand| Operand::from(operand, pcx))
            .collect()
    } else {
        vec![]
    }
