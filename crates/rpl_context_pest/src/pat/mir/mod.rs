...
impl<'pcx> PlaceElem<'pcx> {
    pub fn from_field(field: &'pcx pairs::MirPlaceField) -> Self {
...
impl<'pcx> Place<'pcx> {
    ...
    pub fn from(place: &'pcx pairs::MirPlace<'pcx>, pcx: PatCtxt<'pcx>) -> Self {
        let (base, suffix) = place.get_matched();
        let local = match base.deref() {
            Choice3::_0(local) => Local::from(333_u32), //FIXME
            Choice3::_1(paren) => {
                let (_, place, _) = paren.get_matched();
                return Place::from(place, pcx);
            },
            Choice3::_2(deref) => {
        ...
        let place_elems = suffix
            ...
            .map(|suffix| match suffix.deref() {
                ...
                Choice5::_1(index) => {
                    ...
                    let local = Local::from(333_u32); //FIXME
            ...
            .collect::<Vec<_>>();
        Self::new(local, pcx.mk_slice(&place_elems))
    }
}
...
pub enum RawStatement<'pcx> {
    Assign(Place<'pcx>, Rvalue<'pcx>),
    CallIgnoreRet(Operand<'pcx>, Vec<Operand<'pcx>>),
    Drop(Place<'pcx>),
    Break,
    Continue,
...
impl<'pcx> RawStatement<'pcx> {
    pub fn from(stmt: &'pcx pairs::MirStmt<'pcx>, pcx: PatCtxt<'pcx>) -> Self {
        match stmt.deref() {
            Choice6::_0(call_ignore_ret) => Self::from_call_ignore_ret(call_ignore_ret.get_matched().0, pcx),
            Choice6::_1(drop_) => Self::from_drop(drop_.get_matched().0, pcx),
            Choice6::_2(control) => Self::from_control(control.get_matched().0),
            Choice6::_3(assign) => Self::from_assign(assign.get_matched().0, pcx),
            Choice6::_4(loop_) => Self::from_loop(loop_, pcx),
            Choice6::_5(switch_int) => Self::from_switch_int(switch_int, pcx),
        }
    }

    pub fn from_assign(stmt: &'pcx pairs::MirAssign<'pcx>, pcx: PatCtxt<'pcx>) -> Self {
        let (_, place, _, rvalue_or_call) = stmt.get_matched();
        let place = Place::from(place, pcx);
        let rvalue = match rvalue_or_call.deref() {
            Choice2::_0(_call) => todo!("call in mir assign"),
            Choice2::_1(rvalue) => Rvalue::from(rvalue, pcx),
        };
        Self::Assign(place, rvalue)
    }

    pub fn from_call_ignore_ret(call_ignore_ret: &'pcx pairs::MirCallIgnoreRet<'pcx>, pcx: PatCtxt<'pcx>) -> Self {
        let (_label, _, _, call) = call_ignore_ret.get_matched();
        let (fn_op, _, args, _) = call.get_matched();
        let func = Operand::from_fn_op(fn_op, pcx);
        let args = collect_operands(&args, pcx);
        Self::CallIgnoreRet(func, args)
    }

    pub fn from_drop(drop_: &'pcx pairs::MirDrop<'pcx>, pcx: PatCtxt<'pcx>) -> Self {
        let (_label, _, _, place, _) = drop_.get_matched();
        let place = Place::from(place, pcx);
        Self::Drop(place)
    }

    pub fn from_loop(loop_: &'pcx pairs::MirLoop<'pcx>, pcx: PatCtxt<'pcx>) -> Self {
        let (_, _, block) = loop_.get_matched();
        let statements = block
            .get_matched()
            .1
            .iter_matched()
            .map(|stmt| Self::from(stmt, pcx))
            .collect();
        Self::Loop(statements)
    }

    pub fn from_control(control: &'pcx pairs::MirControl<'pcx>) -> Self {
        let (_label, break_or_continue, _label2) = control.get_matched();
        match break_or_continue.deref() {
            Choice2::_0(_break) => Self::Break,
            Choice2::_1(_continue) => Self::Continue,
        }
    }

    pub fn from_switch_int(switch_int: &'pcx pairs::MirSwitchInt<'pcx>, pcx: PatCtxt<'pcx>) -> Self {
        let (_, _, op, _, _, targets, _) = switch_int.get_matched();
        let operand = Operand::from(op, pcx);
        ...
        targets.iter_matched().for_each(|target| {
            ...
            let statements = Self::from_switch_int_block(block, pcx);
    ...
    pub fn from_switch_int_block(block: &'pcx pairs::MirSwitchBody<'_>, pcx: PatCtxt<'pcx>) -> Vec<Self> {
        let mut stmts = Vec::new();
        match block.deref() {
            Choice4::_0(mir_stmt_block) => {
                return Self::from_mir_stmt_block(mir_stmt_block, pcx);
            },
            Choice4::_1(single_stmt_with_comma) => {
                let (stmt, _) = single_stmt_with_comma.get_matched();
                match stmt {
                    Choice4::_0(call_ignore_ret) => {
                        stmts.push(Self::from_call_ignore_ret(call_ignore_ret, pcx));
                    },
                    Choice4::_1(drop) => stmts.push(Self::from_drop(drop, pcx)),
                    Choice4::_2(control) => stmts.push(Self::from_control(control)),
                    Choice4::_3(assign) => stmts.push(Self::from_assign(assign, pcx)),
                };
            },
            Choice4::_2(loop_) => stmts.push(Self::from_loop(loop_, pcx)),
            Choice4::_3(switch_int) => stmts.push(Self::from_switch_int(switch_int, pcx)),
        }
        stmts
    }

    pub fn from_mir_stmt_block(block: &'pcx pairs::MirStmtBlock<'_>, pcx: PatCtxt<'pcx>) -> Vec<Self> {
        block
            .get_matched()
            .1
            .iter_matched()
...
impl<'pcx> Rvalue<'pcx> {
    fn from(rvalue: &'pcx pairs::MirRvalue<'_>, pcx: PatCtxt<'pcx>) -> Self {
        match rvalue.deref() {
            Choice12::_0(_any) => Rvalue::Any,
            Choice12::_1(cast) => {
                let (operand, _, ty, _, cast_kind, _) = cast.get_matched();
                let operand = Operand::from(operand, pcx);
            ...
            Choice12::_2(rvalue_use) => {
                let operand = match rvalue_use.deref() {
                    Choice2::_0(op) => Operand::from(op.get_matched().1, pcx),
                    Choice2::_1(op) => Operand::from(op, pcx),
                };
                Self::Use(operand)
            },
            Choice12::_3(repeat) => {
            ...
            Choice12::_7(bin_op) => {
                ...
                let lop = Operand::from(lop, pcx);
                let rop = Operand::from(rop, pcx);
            ...
            Choice12::_11(agg) => {
                let (agg_kind, operands) = AggKind::from(agg, pcx);
...
impl<'pcx> Operand<'pcx> {
    pub fn from(op: &'pcx pairs::MirOperand<'_>, pcx: PatCtxt<'pcx>) -> Self {
        match op.deref() {
            Choice6::_0(_any) => Self::Any,
            Choice6::_1(_any_multiple) => Self::Any, // FIXME
            Choice6::_2(meta_var) => Self::from_meta_var(meta_var),
            Choice6::_3(move_) => Self::from_move(move_, pcx),
            Choice6::_4(copy_) => Self::from_copy(copy_, pcx),
            Choice6::_5(konst) => Self::from_constant(konst, pcx),
        }
    }

    pub fn from_meta_var(meta_var: &'pcx pairs::MetaVariable<'_>) -> Self {
        Self::FnPat(Symbol::intern(meta_var.span.as_str()))
    }

    pub fn from_move(move_: &'pcx pairs::MirOperandMove<'_>, pcx: PatCtxt<'pcx>) -> Self {
        Self::Move(Place::from(move_.MirPlace(), pcx))
    }

    pub fn from_copy(copy_: &'pcx pairs::MirOperandCopy<'_>, pcx: PatCtxt<'pcx>) -> Self {
        Self::Copy(Place::from(copy_.MirPlace(), pcx))
    }

    pub fn from_constant(konst: &'pcx pairs::MirOperandConst<'_>, pcx: PatCtxt<'pcx>) -> Self {
        Self::Constant(ConstOperand::from(konst, pcx))
    }

    pub fn from_fn_op(op: &'pcx pairs::MirFnOperand<'_>, pcx: PatCtxt<'pcx>) -> Self {
        match op.deref() {
            Choice5::_0(copy_) => Self::from_copy(copy_.get_matched().1, pcx),
            Choice5::_1(move_) => Self::from_move(move_.get_matched().1, pcx),
...
impl<'pcx> ConstOperand<'pcx> {
    fn from(op: &'pcx pairs::MirOperandConst<'_>, pcx: PatCtxt<'pcx>) -> Self {
    ...
    fn from_type_path(type_path: &'pcx pairs::TypePath<'_>, pcx: PatCtxt<'pcx>) -> Self {
        Self::ZeroSized(PathWithArgs::from_type_path(type_path, pcx))
    }

    fn from_lang_item(lang_item: &'pcx pairs::LangItemWithArgs<'_>, pcx: PatCtxt<'pcx>) -> Self {
        Self::ZeroSized(PathWithArgs::from_lang_item(lang_item, pcx))
    }

    fn from_literal(lit: &'pcx pairs::Literal<'_>) -> Self {
        match lit.deref() {
...
impl<'pcx> AggKind<'pcx> {
    pub fn from(agg: &'pcx pairs::MirRvalueAggregate<'_>, pcx: PatCtxt<'pcx>) -> (Self, List<Operand<'pcx>>) {
        match agg.deref() {
            Choice6::_0(array) => {
                let (_, operands, _) = array.get_matched();
                let operands = collect_operands(&operands, pcx);
                    ...
                    let (symbols, ops): (Vec<Symbol>, Vec<Operand>) = fields
                        .map(|field| {
                            (
                                ...
                                Operand::from(field.MirOperand(), pcx),
                            )
                        })
                        .unzip();
            ...
            Choice6::_5(raw_ptr) => {
                ...
                let ty = Ty::from(ty, pcx);
                let mutability = mutability_from_pair_ptr_mutability(&ptr_mutability);
                let operands = Box::new([Operand::from(op1, pcx), Operand::from(op2, pcx)]);
...
impl<'pcx> MirPatternBuilder<'pcx> {
    ...
    fn mk_raw_stmt(&mut self, kind: RawStatement<'pcx>) -> Location {
        match kind {
            ...
            RawStatement::CallIgnoreRet(func, args) => self.mk_fn_call(func, args.into_boxed_slice(), None),
